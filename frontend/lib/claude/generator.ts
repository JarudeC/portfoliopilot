...
- Concentration strategies (significant weight differences based on conviction)
- Long-only strategies with varying conviction levels

Example format - RETURN RAW SCORES:
function calculateWeights(stockData: any[], dashboardParams: any = {}): number[] {
  const weights = stockData.map(stock => {
    // Calculate your score/weight for this stock
    // Examples: momentum score, volatility score, ranking score
    // If stock A scores 0.8 and stock B scores 0.4, stock A gets 2x the weight
    
    let score = 0; // Your analysis here
    return score; // Return raw score - NO normalization
  });
  
  return weights; // Return raw weights - do NOT normalize or sum to 1
}
...
function validateAndFixCode(code: string, mode: 'forecast' | 'backtest'): ValidationResult {
  ...
  // Skip client-side syntax validation - let the browser-based Babel transpilation handle it
  // The regex-based TypeScript stripping is too crude and creates syntax errors
  // Babel transpilation in executeUserCode will handle TypeScript conversion properly
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      if (mode === 'forecast') {
        ...
        // Return raw weights as generated by AI - DO NOT normalize to sum to 1
        // The prompt explicitly instructs the AI to return differentiated raw scores
        // Normalization happens later in the dashboard if needed for long-only strategies
