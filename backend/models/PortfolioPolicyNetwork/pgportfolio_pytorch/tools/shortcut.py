...
from pgportfolio_pytorch.trade.backtest import BackTest
...
def execute_backtest(algo, config):
    """
    aim: to execute backtest for every algorithms 
    @:param algo: string representing the name the name of algorithms
    @:return: numpy array of portfolio changes
    """
    agent, agent_type, net_dir = _construct_agent(algo) # construct agent
    backtester = BackTest(config, agent=agent, agent_type=agent_type, net_dir=net_dir)  #initialization，and construct the backtest dataset  
    
    backtester.start_trading()       # 
    return backtester.test_pc_vector, backtester.turn_over


def _construct_agent(algo):
    """
    aim: to construct agent
    """
    if algo.isdigit():
        agent = None
        agent_type = "nn"
