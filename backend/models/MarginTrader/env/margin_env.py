...
class MarginTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.day = day
        self.df = df.copy()  # Fix: Make a copy to avoid SettingWithCopyWarning
    ...
    def _buy_long_stock(self, index, action): 
        def _do_long_buy():
            if self._check_long_maintenance() > self.maintenance:
                # Calculate maximum shares that can be bought based on current cash
                max_possible_shares = self.state[0] / (
                    self.state[index + 2*3] * (1 + self.buy_cost_pct[index])
                )
                buy_num_shares = min(max_possible_shares, abs(action))
                
                if buy_num_shares > 0:
                    transaction_cost = self._apply_transaction_costs(index, buy_num_shares, True)
                    buy_amount = self.state[index + 2*3] * buy_num_shares
                    
                    # Update state
                    self.state[0] -= (buy_amount + transaction_cost)  # Reduce cash
                    self.state[2] -= transaction_cost  # Track trading costs
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            price_slice = np.array(self.data.close.tolist())
            hold_slice  = slice(self.stock_dim + 6, self.stock_dim*2 + 6)
            pos_slice   = np.array(self.state[hold_slice])
        
            gross_long  = np.sum(np.maximum(pos_slice, 0) * price_slice)
            gross_short = np.sum(np.abs(np.minimum(pos_slice, 0) * price_slice))
            gross       = gross_long + gross_short
            
            equity      = max(self.state[2] + self.state[5], 1)               # long_eq + short_eq
            allowable   = self.max_leverage * equity
            if gross > allowable:
                scale   = allowable / gross
                new_pos = pos_slice * scale
                # 1) scale positions
                self.state[hold_slice] = new_pos.tolist()
                # 2) compute difference and credit cash/limit
                share_diff  = pos_slice - new_pos
                delta_long  =  sum(np.maximum(share_diff, 0) * price_slice)
                delta_short =  sum(abs(np.minimum(share_diff, 0)) * price_slice)
                self.state[0] += delta_long    # add proceeds to cash
                self.state[3] += delta_short   # restore limit for shorts


            cur_date_info = self.dates[self.day]
            ...
            equity_base = max(begin_total_asset, self.initial_amount * 0.25)  # floor at 25% NAV
            
            # Fix: Add NaN protection for reward calculation
            if np.isnan(begin_total_asset) or np.isnan(end_total_asset) or equity_base <= 0:
                raw_ret = 0.0
            else:
                raw_ret = (end_total_asset - begin_total_asset) / equity_base
                
            # Ensure raw_ret is not NaN or infinite
            if np.isnan(raw_ret) or np.isinf(raw_ret):
                raw_ret = 0.0
                
            self.reward = float(np.clip(raw_ret, -1.0, 1.0)) * self.reward_scaling             # clip to ±1 
            
            if len(self.asset_memory)>=5:
                daily_return = pd.Series(self.asset_memory[-5:]).pct_change(1)
                std_return = daily_return.std()
                # Fix: Prevent division by zero and handle NaN values
                if std_return > 1e-8 and not np.isnan(std_return):
                    sharpe = (252**0.5) * daily_return.mean() / std_return
                    # Clip sharpe to prevent extreme values
                    sharpe = np.clip(sharpe, -10.0, 10.0)
                else:
                    sharpe = 0.0
                # Ensure sharpe is not NaN
                if np.isnan(sharpe):
                    sharpe = 0.0
            else:
                sharpe = 0.0
            ...
            # Final NaN protection - ensure reward is always finite
    ...
    def _update_state(self):
        ...
        # compute actual short equity change
        ...
        # floor negative short equity instead of exiting
        if short_actual_equity < 0:
            short_actual_equity = 0

        # --- rebuild state vector -----------------------------------------
        state = (
            [long_cash, loan, long_equity]              # 3 long-account scalars
            + [limit, credit, short_equity]             # 3 short-account scalars
            + self.data.close.values.tolist()           # prices   (stock_dim)
            + list(self.state[(self.stock_dim + 2*3):(self.stock_dim*2 + 2*3)])  # holdings (stock_dim)
            + sum(
                (self.data[tech].values.tolist() for tech in self.tech_indicator_list),
                [],
            )                                           # tech indicators (optional)
        )

        # --- numerical stabilisation (fair bounds) ------------------------
        CLIP_LIMIT = self.initial_amount * 10          # cap at ±10× initial equity
        state = np.nan_to_num(state, nan=0.0)          # remove NaNs
    ...
    def _apply_transaction_costs(self, index, num_shares, is_buy):
        """Apply transaction costs based on the turnover, following the same methodology as other models."""
        price = self.state[index + 2*3]
        cost_pct = self.buy_cost_pct[index] if is_buy else self.sell_cost_pct[index]
        
        # Calculate turnover (absolute change in position value)
        turnover = abs(price * num_shares)
