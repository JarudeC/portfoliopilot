...
class BoundedKMeansClustering:
    ...
    def fit(self, X: np.array, weights: np.array, dist_array: np.array = None) -> tuple[float, list[list[int]]]:
        self.n_points = X.shape[0]
        dist_array = dist_array if dist_array is not None else ClustersUtils.compute_dist_array(X)

        costs, clusters = zip(*[
            self.fit_one_iteration(X, weights, dist_array)
            for _ in range(self.n_init)
        ])

        if all(np.isnan(np.array(costs))):
            self.n_points = None
            return np.nan, [[]]  # return an empty cluster structure to avoid TypeError
